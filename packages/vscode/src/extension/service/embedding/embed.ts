...
export class VectraManager {
    ...
    async getVector(text: string): Promise<number[]> {
        try {
            const response = await fetch('https://codemuse-app--generate-embedding.modal.run', {
                method: 'POST',
                headers: {
    ...
    async query(text: string) {
        ...
        if (results.length > 0) {
            for (const result of results) {
                console.log(`[${result.score}] ID: ${result.item.metadata.id}, Hash: ${result.item.metadata.hash}, Path: ${result.item.metadata.path}`);
            }
        } else {
