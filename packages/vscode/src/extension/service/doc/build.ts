...
function inputDefinesChildren(
  code: string,
  nodeObject: LocalGraphNode,
  childrenNodeObjects: Set<LocalGraphNode>
): string {
  const locationsAndDocumentations: [
    number,
    number,
    string | undefined,
    number,
  ][] = [...childrenNodeObjects].map((e: LocalGraphNode) => {
    if (
      nodeObject.symbol ==
      "scip-python python rtcmd-api-server indexer `server.service.modules.aluminium.api.quote_viewset`/QuoteViewSet#"
    ) {
      console.log(e);
      console.log(buildInput.getLineOfSignature(e.content.split("\n")));
    }
    const lineOfSignature: number = buildInput.getLineOfSignature(
      e.content.split("\n")
    );

    const lineOfSignatureInCode =
      e.range[0] + lineOfSignature - nodeObject.range[0];
    return [
      e.range[0] + lineOfSignature + 1,
      e.range[2],
      e.documentation,
      lineOfSignatureInCode,
    ];
  });

  //console.log(buildInput.replaceCodeByDocumentation(nodeObject.file, code, locationsAndDocumentations))

  return buildInput.replaceCodeByDocumentation(
    nodeObject.file,
    code,
    locationsAndDocumentations
  );
}

function inputUsesChildren(
  code: string,
  nodeObject: LocalGraphNode,
  locationsAndDocumentations: Set<[number, string, string | undefined]>
): string {
  if (locationsAndDocumentations.size === 0) {
    return code;
  }
  return buildInput.insertDocumentationInCode(
    code,
    nodeObject.file,
    locationsAndDocumentations
  );
...
export async function documentNode(graph: Graph, node: string, token: string) {
  ...
  if (outBoundEdges.length == 0) {
    ...
    let locationsAndDocumentationsUses: Set<
    ...
    graph.forEachOutboundEdge(node, (_, attr, from, to) => {
      ...
      if (type == "defines") {
        ...
        locationsAndDocumentationsUses.add([
          edgeAttribute.at[0],
          toObject.symbol,
    ...
    if (outBoundDefinesChildren.size > 0) {
      processedContent = inputDefinesChildren(
        processedContent,
        currentNodeObject,
        outBoundDefinesChildren
      );
    }
    if (locationsAndDocumentationsUses.size > 0) {
      processedContent = inputUsesChildren(
        processedContent,
        currentNodeObject,
        locationsAndDocumentationsUses
      );
    }
